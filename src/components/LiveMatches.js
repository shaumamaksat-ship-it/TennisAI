...
export default function LiveMatches() {
  ...
  const load = async () => {
    ...
    const nextPhotos = {};
    const keys = Object.keys(needed).slice(0, 24);
  ...
  if (loading) {
    return <div style={{ padding: "0 12px", color: "#94a3b8" }}>Загрузка live-матчей...</div>;
  }

  return (
    <div style={{ padding: "0 12px 20px" }}>
      <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 10 }}>
        🔥 LIVE MATCHES ({filtered.length})
      </div>
      ...
      {!groups.length && (
        <div style={{ color: "#94a3b8", fontSize: 13, padding: "8px 0" }}>Ничего не найдено</div>
      )}
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                        <img
                          ...
                          onClick={function(e) {
                            e.stopPropagation();
                            if (m.first_player_key) navigate("/player/" + m.first_player_key);
                          }}
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <img
                          ...
                          onClick={function(e) {
                            e.stopPropagation();
                            if (m.second_player_key) navigate("/player/" + m.second_player_key);
                          }}
                          style={{ width: 28, height: 28, borderRadius: "50%", objectFit: "cover", cursor: "pointer" }}
                        />
